    ...
    >
      <div
        className={`w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl border-4
          ${isBad ? 'bg-red-950 border-red-500' : isGood ? 'bg-green-950 border-green-500' : 'bg-yellow-950 border-yellow-500'}`}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 text-center">
          <div className="text-6xl mb-2 animate-bounce">{isBad ? '🚨' : isGood ? '🎉' : '⚡'}</div>
          <p className={`text-xs font-black uppercase tracking-[0.2em] mb-2
            ${isBad ? 'text-red-400' : isGood ? 'text-green-400' : 'text-yellow-400'}`}>
            {isBad ? '¡ A L E R T A !' : isGood ? '¡ B U E N A S   N O T I C I A S !' : '¡ E V E N T O !'}
          </p>
        ...
        {event.detail && (
          <div className={`mx-4 mb-4 rounded-2xl p-4 text-sm space-y-2
            ${isBad ? 'bg-red-900/50 border border-red-700' : isGood ? 'bg-green-900/50 border border-green-700' : 'bg-yellow-900/40 border border-yellow-700'}`}>
            <p className="text-indigo-200 leading-relaxed">{event.detail}</p>
            {event.impactLabel && (
              <p className={`font-bold text-sm ${isBad ? 'text-red-300' : isGood ? 'text-green-300' : 'text-yellow-300'}`}>
                {isGood ? '✨' : '💥'} {event.impactLabel}
              </p>
            )}
            {event.personName && (
        ...
        <div className="px-6 pb-6 text-center">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-base transition active:scale-95
              ${isBad ? 'bg-red-600 hover:bg-red-500 text-white' : isGood ? 'bg-green-600 hover:bg-green-500 text-white' : 'bg-yellow-500 hover:bg-yellow-400 text-black'}`}>
            ✅ Enterado — continuar
          </button>
